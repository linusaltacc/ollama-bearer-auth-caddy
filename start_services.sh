...
trap "echo 'Received shutdown signal, stopping all services...' >> $LOG_FILE; kill $CADDY_PID $UVICORN_PID; exit 0" SIGTERM SIGINT
...
while true; do
    ...
    if ! ps -p $UVICORN_PID > /dev/null; then
        ...
        uvicorn app.main:app --host 0.0.0.0 --port 9090 >> "$UVICORN_LOG_FILE" 2>&1 &
        UVICORN_PID=$!
    fi
    sleep 1
done